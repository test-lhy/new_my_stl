...
class EpollLoop {
 public:
  void AddListener(std::coroutine_handle<EpollFilePromise> promise) const {
    /// 设置监听的时候会保存该promise，使得在读取后能回到该协程
    epoll_event event{};
    event.data.ptr = &promise;
    /// 处理什么样的事件
    event.events = promise.promise().events;
    epoll_ctl(epfd, EPOLL_CTL_ADD, promise.promise().FileNo, &event);
  }
  void TryRun() {
    epoll_event events[10];
    int nfds = epoll_wait(epfd, events, 10, -1);
    for (int i = 0; i < nfds; i++) {
      auto promise = static_cast<std::coroutine_handle<EpollFilePromise>*>(events[i].data.ptr);
      epoll_ctl(epfd, EPOLL_CTL_DEL, promise->promise().FileNo, nullptr);
      promise->resume();
    }
  }
...
class EpollFileAwaiter {
 public:
  ...
  void await_suspend(std::coroutine_handle<EpollFilePromise> previous_handle) noexcept {
    ...
    GetEpollLoop().AddListener(previous_handle);
  }
