...
class hash {
 ...
 public:
  explicit hash(size_t, HashFuncType<T>& hash_func = TypeTraits<T>::hash_func);
...
bool hash<T, U>::equal(const T& a, const T& b) {
  return hash_func_(a, mod_value) == hash_func_(b, mod_value);
}
template <typename T, typename U>
hash<T, U>::~hash() = default;
template <typename T, typename U>
hash<T, U>::hash(HashFuncType<T>& hash_func) {
  hash_table_.reserve(mod_value + 5);
  hash_func_ = hash_func;
}

template <typename T, typename U>
hash<T, U>::hash(size_t size, HashFuncType<T>& hash_func) : hash(hash_func) {
  mod_value = GetModValue(size);
  hash_table_.reserve(mod_value + 5);
  hash_func_ = hash_func;
}
template <typename T, typename U>
void hash<T, U>::insert(const T& key, const U& value) {
  hash_table_[hash_func_(key, mod_value)].push_back({key, value});
}
};      // namespace lhy
#endif  // MY_STL_HASH_H