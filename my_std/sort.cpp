...
size_t GetMinRun(size_t size) {
  ...
  for (auto* element = (int64_t *)(binary_size.rbegin()); element != binary_size.rbegin() - 6; element--) {
    binary_minrun.push_back(*element);
  }
  for (auto* element =  (int64_t *)(binary_size.rbegin() - 6); element != binary_size.rend(); --element) {
    if (*element == 1ll) {
